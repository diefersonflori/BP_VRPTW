...
# Templates HTML mantidos fora do Python. Para cada instancia, a main copia o
# template correspondente e altera somente a referencia ao arquivo JS local.
PASTA_PLOT_JS = BASE_DIR / "PlotJS"
PASTA_TEMPLATES_HTML = PASTA_PLOT_JS / "basePadrao"
CONFIG_PLOT_JS = {
    "construtiva": {
        "template": PASTA_TEMPLATES_HTML / "gantt_petro_construtiva.html",
        "html_saida": "gantt_petro_construtiva.html",
        "js_saida": "visualizacao_dados_construtiva.js",
    },
    "bp": {
        "template": PASTA_TEMPLATES_HTML / "gantt_petroBp.html",
        "html_saida": "gantt_petroBp.html",
        "js_saida": "visualizacao_dadosBP.js",
    },
    "exato": {
        "template": PASTA_TEMPLATES_HTML / "gantt_petro_exato.html",
        "html_saida": "gantt_petro_exato.html",
        "js_saida": "visualizacao_dados_exato.js",
    },
}
...
def validar_templates_plotjs():
    faltantes = [
        str(config["template"])
        for config in CONFIG_PLOT_JS.values()
        if not config["template"].is_file()
    ]
    if faltantes:
        lista = "\n".join(f"  - {caminho}" for caminho in faltantes)
        raise FileNotFoundError(
            "Templates HTML nao encontrados em PlotJS/basePadrao:\n" + lista
        )


def copiar_template_apontando_para_js(
    template_html: Path,
    destino_html: Path,
    nome_js: str,
):
    """Copia o HTML e troca somente a linha script.src do arquivo de dados.

    O HTML completo continua mantido no arquivo-template. A main nao incorpora
    o codigo da pagina; ela apenas aponta a copia para o JS da mesma pasta.
    """
    texto = template_html.read_text(encoding="utf-8")

    # Linha esperada nos templates, por exemplo:
    # script.src = "petroConstr.js?v=" + Date.now();
    padrao = re.compile(
        r'(?m)^(?P<indent>\s*)script\.src\s*=\s*["\'][^"\']+["\']'
        r'\s*\+\s*Date\.now\(\)\s*;'
    )
    substituicao = (
        rf'\g<indent>script.src = "{nome_js}?v=" + Date.now();'
    )
    texto_novo, quantidade = padrao.subn(substituicao, texto, count=1)

    if quantidade != 1:
        raise RuntimeError(
            f"Nao encontrei uma unica linha script.src dinamica em {template_html}. "
            "Mantenha no template uma linha como: "
            'script.src = "arquivo.js?v=" + Date.now();'
        )

    destino_html.write_text(texto_novo, encoding="utf-8")


def exportar_pacote_plotjs(
    solucao,
    inst,
    nome_base: str,
    tipo: str,
):
    """Gera o par HTML + JS dentro de PlotJS/<nome_da_instancia>."""
    config = CONFIG_PLOT_JS[tipo]
    pasta_instancia = PASTA_PLOT_JS / nome_base
    pasta_instancia.mkdir(parents=True, exist_ok=True)

    caminho_js = pasta_instancia / config["js_saida"]
    caminho_html = pasta_instancia / config["html_saida"]

    dados = solucao.exportar_visualizacao(
        inst,
        tipo,
        str(caminho_js),
    )
    if dados is None or not caminho_js.is_file():
        raise RuntimeError(
            f"A exportacao JS de {tipo} nao foi concluida para {nome_base}."
        )

    copiar_template_apontando_para_js(
        config["template"],
        caminho_html,
        config["js_saida"],
    )

    print(f"[PLOTJS] {tipo}: {caminho_html}")
    return caminho_html, caminho_js
...
def main():
    ...
    print(f"Templates HTML: {PASTA_TEMPLATES_HTML}")
    print(f"Pacotes HTML + JS: {PASTA_PLOT_JS / '<nome_da_instancia>'}")
    ...
    print(f"Pacotes HTML + JS: {PASTA_PLOT_JS}")
