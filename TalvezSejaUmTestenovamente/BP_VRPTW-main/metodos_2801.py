...
class Metodos:
    ...
    def resolver_no_com_pool(self, inst, sol_pool, no_bp, tipo_geracao="PD"):
        ...
        for k in sol_pool.rotas.keys():
            ...
            # Parte A: coeficientes (debugável)
            coef_arco = {}  # (i,j) -> list size nrotas com 0/1
            nrotas = len(sol_pool.rotas[k]['sequencia_rota'])
        ...
        while True:
            ...
            for k in sol_pool.rotas.keys():
                ...
                # NOVO: mu_arc = dual das restrições de arco do nó (só as do veículo k)
                mu_arc = {}
                ...
                if tipo_geracao == "PD":
                    ...
                    nova_rota, custo_red = self.SUB_PROG_DINCPPantig(
                    #ova_rota, custo_red = self.SUB_PROG_DINLivre(
                ...
                if custo_red < -1e-6:
                    ...
                    # NOVO: restrições de arco do nó (somente as de k)
    ...
    def SUB_PROG_DINCPP(self, inst, pi, sigma_k, k,
        ...
        if arcos_proibidos is None: arcos_proibidos = set()
        if arcos_fixados is None: arcos_fixados = set()
        if mu_arc is None: mu_arc = {}
        ...
        F = np.array(list(arcos_proibidos), dtype=np.int32).reshape(-1, 2) if arcos_proibidos else np.zeros((0, 2),
                                                                                                            np.int32)
        FX = np.array(list(arcos_fixados), dtype=np.int32).reshape(-1, 2) if arcos_fixados else np.zeros((0, 2),
                                                                                                         np.int32)

        # mu_arc dict {(i,j):mu} -> mm x 3 float64
        if mu_arc:
            MU = np.array([(float(i), float(j), float(v)) for (i, j), v in mu_arc.items()],
    ...
    def SUB_PROG_DINCPPantig(self, inst, pi, sigma_k, k, arcos_proibidos=None, arcos_fixados=None):
        import sys
        import numpy as np
        from pathlib import Path
