...
class Metodos:
    ...
    def SUB_PROG_DIN_PETRO_CPP(
            self, inst, pi, sigma_k, k,
            NO_BP=None, mu_arc=None, eps=1e-6,
            timeout_s=15,
            max_labels_por_no=1_000_000_000
    ):
        ...
        kwargs_cpp = {
            ...
            "req_i": req_i,
            "req_j": req_j,
            "max_labels_por_no": int(max_labels_por_no),
            "eps": float(eps),
        }
        ...
        print(
            f"[PETRO CPP] k={k} | limite={timeout_s}s | "
            f"max_labels={max_labels_por_no} | "
            f"tempo={time.time() - t0_cpp:.2f}s | "
            f"timeout={self._ultimo_timeout_cpp}"
        )
    ...
    def _petro_pricing_exato(
        ...
        if usa_cpp:
            return self.SUB_PROG_DIN_PETRO_CPP(
                inst,
                pi,
                sigma_k,
                k,
                ...
                mu_arc=mu_arc,
                timeout_s=timeout_s,
                max_labels_por_no=max_labels_por_no
            )
        ...
        while True:
            ...
            if not novas_colunas:
                ...
                if getattr(no_bp, "pricing_timeout", False):
                    ...
                    print(
                        "[FALHA CG] Nenhuma coluna retornada, mas o pricing "
                        "completo terminou por timeout."
                    )
            ...
            if melhor_rota is None:
                if hasattr(inst, "dados_petro"):
                    ...
                    if self._ultimo_timeout_cpp:
                        ...
                        print(
                            f"[PETRO] PD completa excedeu 15s "
                            f"no veículo {k}; convergência não certificada."
                        )
    ...
    def chamar_cpp_timeout(self, func_cpp, args=(), kwargs=None, timeout=600):
        ...
        # Ainda está executando: ultrapassou o limite.
        if p.is_alive():
        ...
        # O processo terminou dentro do limite.
        try:
